...
const AddLocation = ({setShowAdd}) => {
    ...
    const handleUpload = async (itemid) => {
        ...
        const res = await axios.post(`${import.meta.env.VITE_API_URL}/admin/CMS/locations/upload-image/${itemid}`, data, { withCredentials: true });
        } catch (error) {
        alert(error.message);
        } 
    };
    ...
    const addLocation = async() => {
        ...
        try {
            ...
            if (file === null) {
                setIsImgError(true);
                setImgErrorMessage("Input an image");
                end = true;
            }
            ...
            const locationid = await response.json();  // now the actual id, e.g. 42

            console.log("CCC");

            if (response.ok) {
                await handleUpload(locationid);
                console.log("DDD");
                setShowAdd(false);
            }
        ...
        >
            ...
            <form className='w-full flex gap-5 flex-col'>
                ...
                {!is24hrs && (
                    <>
                        {/* Open Time */}
                        <Field.Root invalid={isOpenTimeError} className='w-full' required>
                        ...
                        {/* Close Time */}
                        <Field.Root invalid={isCloseTimeError} className='w-full' required>
                ...
                {/* Image */}
                <Field.Root className='w-full' required invalid={isImgError}>
                ...
                {/* Directions Link */}
                <Field.Root invalid={isDirectionsError} required className='w-full'>
            ...
            <Flex className='justify-end gap-3'>
                ...
                <Button loading={buttonLoading} disabled={buttonLoading} className='editButton' style={{ background:'#4BB543' }} onClick={() => addLocation()}>
                    Add
                </Button>
