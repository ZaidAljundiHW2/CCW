import { Flex, Field, Input, Span, Button, Textarea } from '@chakra-ui/react'
...
const EditMenuItem = ({menuitem, setShowMenuItemEdit, categories}) => {
    ...
    const handleUpload = async (itemid, currImgURL) => {
        ...
        const res = await axios.post(`${import.meta.env.VITE_API_URL}/upload/menu/item/${itemid}`, data, { withCredentials: true });
        } catch (error) {
        alert(error.message);
        } 
    };
    ...
    const handleUpdate = async (item, categories) => {
        ...
        try {
            ...
            if (file !== null) {
                await handleUpload(fooditemid, item.foodimage);
